...
class FunctionCalculator:
    ...
    def median(self, data):
        """Медиана (без import statistics)"""
        if not data:
            return 0
        return median(data)

    def mode(self, data):
        """Мода (без Counter) — наиболее часто встречающееся значение"""
        if not data:
            return 0
        return mode(data)
    ...
    def print_statistics(self, values):
        ...
        print("\n" + "=" * 60)
        print("Статистические параметры последовательности значений:")
        print("=" * 60)
        print(f"Среднее арифметическое: {self.mean(values):.8f}")
        print(f"Медиана:               {self.median(values):.8f}")
        print(f"Мода:                  {self.mode(values):.8f}")
        print(f"Дисперсия (выборочная):{self.variance(values):.8f}")
        print(f"СКО (выборочное):      {self.std_dev(values):.8f}")
        print("=" * 60)
    ...
    def plot_graphs(self):
        ...
        print(f"Plot saved as '{self.PLOT_FILE}'")
        plt.show()
